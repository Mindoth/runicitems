...
public class ShootSpell {
    ...
    public static void shootMagicSpark(LivingEntity owner, Entity caster, IItemHandler itemHandler, int slot, HashMap<Item, Integer> effects) {
        ...
        MagicSparkEntity magicSpark = new MagicSparkEntity(level, owner, caster, itemHandler, slot, effects);
        if ( !SpellBuilder.getGravity(effects) ) {
            magicSpark.setNoGravity(true);
        }

        if ( caster != owner ) {
            magicSpark.setPos(center);
        }
        magicSpark.shootFromRotation(caster, caster.getXRot(), caster.getYRot(), 0F, 1.5F, 1.0F);

        level.addFreshEntity(magicSpark);
