...
public class ShootSpell {
    ...
    public static void shootMagic(LivingEntity owner, Entity caster, IItemHandler itemHandler, int slot, HashMap<Item, Integer> effects, Item rune) {
        Level level = caster.level;
        Vec3 center = CommonEvents.getEntityCenter(caster);
        ProjectileBaseEntity projectile = new ProjectileBaseEntity(level, owner, caster, itemHandler, slot, effects, rune);
    ...
    public static void summonFamiliar(LivingEntity owner, Entity caster, IItemHandler itemHandler, int slot, HashMap<Item, Integer> effects, Item rune) {
        Level level = caster.level;
        Vec3 center = CommonEvents.getEntityCenter(caster);
        FamiliarBaseEntity familiar = new FamiliarBaseEntity(level, owner, caster, itemHandler, slot, effects, rune);
        if ( !SpellBuilder.getGravity(effects) ) {
        ...
        if ( SpellBuilder.getSpeed(effects, 0) == 0 ) {
            familiar.setDeltaMovement(0, 0, 0);
        }
